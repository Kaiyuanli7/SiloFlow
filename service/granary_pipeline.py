...
def run_complete_pipeline(
    ...
    data_paths = None  # Use centralized data paths
) -> dict:
    ...
    try:
        ...
        if should_train:
            ...
            # Save the final model with compression
            model_path.parent.mkdir(exist_ok=True, parents=True)
            from granarypredict.compression_utils import save_compressed_model
            save_compressed_model(final_model, model_path, compression_level=6, use_gzip=True)
...
def _preprocess_silos(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rename_map = {"x": "grid_x", "y": "grid_y", "z": "grid_z"}
...
def main():
    ...
    if args.command == "ingest":
        ...
        # 1. Standardize columns (rename x/y/z to grid_x/grid_y/grid_z)
        rename_map = {"x": "grid_x", "y": "grid_y", "z": "grid_z"}
        for old, new in rename_map.items():
            if old in df.columns and new not in df.columns:
                df.rename(columns={old: new}, inplace=True)
        ...
        def clean_and_reorder(df):
            # First, ensure grid_x and grid_y exist with correct names
            rename_map = {"x": "grid_x", "y": "grid_y", "z": "grid_z"}
            for old, new in rename_map.items():
                if old in df.columns and new not in df.columns:
                    df.rename(columns={old: new}, inplace=True)
        ...
        save_compressed_model(model, model_path, compression_level=6, use_gzip=True)
        ...
        # Load the trained model with compression support
