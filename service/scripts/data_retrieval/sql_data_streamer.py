...
class SQLDataStreamer:
    ...
    def __init__(self, config_path: str = "streaming_config.json"):
        ...
        # New memory safety parameters
        self.max_records_per_batch = self.config['processing'].get('max_records_per_batch', 100000)
        self.memory_safety_threshold = self.config['processing'].get('memory_safety_threshold_percent', 60)
        self.batch_timeout_seconds = self.config['processing'].get('batch_timeout_seconds', 300)
    ...
    def get_silo_data_chunked(self, heap_id: str, sub_table_id: int, start_date: datetime, end_date: datetime) -> Generator[pd.DataFrame, None, None]:
        ...
        days_per_chunk = max(1, min(30, total_days // 10))  # Process 1-30 days at a time
        ...
        while current_start < end_date:
            ...
            try:
                ...
                if not self.check_memory_usage():
                    logger.warning(f"Memory usage high before processing chunk {chunk_count + 1}, pausing...")
                    gc.collect()
                    time.sleep(5)
    ...
    def collect_data_in_batches(self, granaries_df: pd.DataFrame, start_date: datetime, end_date: datetime, granary_filter: Optional[str] = None, batch_size: Optional[int] = None) -> Generator[pd.DataFrame, None, None]:
        """
        ...
        for silo_idx, (_, silo_info) in enumerate(granaries_df.iterrows()):
            try:
                ...
                # Check memory usage before processing each silo
                if not self.check_memory_usage():
                    logger.warning("Memory usage high, pausing for garbage collection...")
                    gc.collect()
                    time.sleep(5)
                    
                    # If still high, force a batch yield
                ...
                # This ensures we get ALL available data for each silo
    ...
    def _stream_large_dataset(self, granaries_df: pd.DataFrame, start_date: datetime, end_date: datetime, granary_filter: Optional[str] = None, run_pipeline: bool = True, size_estimate: Optional[Dict] = None) -> Dict:
        """
        ...
        try:
            for batch_df in self.collect_data_in_batches(granaries_df, start_date, end_date, granary_filter, batch_size):
                batch_count += 1
                ...
                # Check memory usage
                if not self.check_memory_usage():
                    logger.warning("Memory usage high during streaming, pausing...")
                    time.sleep(10)
                    gc.collect()
            ...
            return {'success': False, 'error': str(e), 'processing_mode': 'streaming'}
    ...
    def stream_all_data(self, start_date: datetime, end_date: datetime, run_pipeline: bool = True, granary_filter: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            # Step 3: Collect all raw data from database
            logger.info("Step 3: Collecting all raw data from database...")
            # Ensure granaries_df is properly typed as DataFrame
            granaries_df = pd.DataFrame(granaries_df).reset_index(drop=True)
            
            # Estimate data size and get recommended batch size
            size_estimate = self.estimate_data_size(granaries_df, start_date, end_date)
            logger.info(f"Estimated data size: {size_estimate['total_estimated_records']:,}, Recommended batch size: {size_estimate['recommended_batch_size']:,}")
            ...
            if size_estimate['recommended_processing_mode'] == 'streaming':
                ...
                return self._stream_large_dataset(granaries_df, start_date, end_date, granary_filter, run_pipeline, size_estimate)
            
            # Collect data in batches for smaller datasets
            raw_data_batches = list(self.collect_data_in_batches(granaries_df, start_date, end_date, granary_filter, size_estimate['recommended_batch_size']))
            ...
            logger.info(f"Step 3 completed: Collected {len(raw_data)} total records")
            
            # Step 4: Process raw data directly through ingestion pipeline (optimized - no temp CSV)
            logger.info("Step 4: Processing raw data directly through ingestion pipeline...")
...
def main():
    ...
    try:
        ...
        results = streamer.stream_all_data(
            ...
            granary_filter=args.granary
        )
        
        if results['success']:
            logger.info("Data streaming completed successfully!")
            logger.info(f"Total records collected: {results['total_records_collected']}")
            logger.info(f"Granaries with new data: {len(results['granaries_with_new_data'])}")
            if results['granaries_with_new_data']:
                logger.info(f"Granaries processed: {', '.join(results['granaries_with_new_data'])}")
            if results.get('pipeline_results'):
                successful_pipelines = sum(1 for r in results['pipeline_results'].values() if r.get('success', False))
                logger.info(f"Pipeline processing: {successful_pipelines}/{len(results['pipeline_results'])} successful")
        else:
