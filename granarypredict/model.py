...
def save_model(model: Any, name: str = "rf_model.joblib", use_compression: bool = True) -> Path:
    path = MODELS_DIR / name
    if use_compression:
        save_compressed_model(model, path, compression_level=6, use_gzip=True)
    else:
...
def load_model(path: str | Path, use_compression: bool = True) -> Any:
    path = Path(path)
    if use_compression and (path.suffix == '.gz' or '.compressed' in path.name):
        model = load_compressed_model(path, use_gzip=True)
    else:
