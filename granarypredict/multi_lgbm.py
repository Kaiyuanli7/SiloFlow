...
class AnchorDayEarlyStoppingCallback:
    ...
    def __call__(self, env):
        ...
        try:
            ...
            if self.verbose:
                print(f"Custom early stopping failed: {e}. Falling back to standard early stopping.")
            return None
    
    def _compute_anchor_mae(self, env) -> float:
        """Compute anchor-day 7-day consecutive MAE with coherence penalty.
        
        OPTIMIZED: Uses vectorized operations for faster computation.
        NEW: Includes coherence penalty for realistic temperature transitions.
        """
        try:
            ...
            # OPTIMIZATION: Vectorized MAE computation across all horizons
            ...
            # OPTIMIZATION: Vectorized mask and MAE computation
            ...
            if valid_mask.any():
                ...
                main_mae = float(np.mean(horizon_maes)) if horizon_maes else float('inf')
                
                # Return main MAE (coherence is now handled post-processing)
                return main_mae
            else:
                return float('inf')
    ...
    def fit(
        ...
        *,
        ...
        if False and n_outputs > 1: # Removed coherence constraint parameters
            ...
            print(f"⚠️  COHERENCE SKIPPED: Single-output model (n_outputs={n_outputs})")
        else:
        ...
        for idx in range(n_outputs):
            ...
            if eval_set is not None:
                ...
                if use_anchor_stopping and idx == 0:  # Use anchor stopping for first horizon only
    ...
    def predict(self, X: Union[pd.DataFrame, np.ndarray]) -> np.ndarray:
        ...
        if len(preds) > 1:
            ...
            if self.uncertainty_estimation:
                ...
                print("⚠️ UNCERTAINTY DISABLED: Returning raw predictions without uncertainty estimates")
                try:
                    import streamlit as st
                    st.toast("⚠️ Uncertainty estimation disabled - no confidence intervals available", icon="⚠️")
                except:
                    pass
                return raw_predictions
        else:
