...
"""Utility helpers for data ordering & grouping (deployment edition)."""
...
_SORT_HIERARCHY: List[str] = [
    ...
    "grid_y",
    "grid_z",
    "detection_time",  # timestamp to keep chronological order
]


def comprehensive_sort(df: pd.DataFrame) -> pd.DataFrame:
    """Return *df* sorted according to canonical hierarchy."""
...
def assign_group_id(df: pd.DataFrame, *, col_name: str = "_group_id") -> pd.DataFrame:
    """Add a column uniquely identifying a physical silo."""
